...
def main():
    ...
    # plot lunar lander results
    train = np.load('log/lander/results_train1.npy')
    x = np.arange(train.shape[0])
    y = train[:,0]
    y = moving_average(y, 5)
    ...
    y = train[:,0]
    y = moving_average(y, 5)
    plt.plot(x, y, label='train2')

    # plot lunar lander results
    train = np.load('log/lander/results_train3.npy')
    x = np.arange(train.shape[0])
    y = train[:,0]
    y = moving_average(y, 5)
    ...
    plt.plot(x, y, label='evaluation')
    """
    evaluation = np.load('log/lander/results_eval1.npy')
    x = np.arange(evaluation.shape[0])*25
    y = evaluation[:,0]
    y = moving_average(y, 4)
    ...
    y = evaluation[:,0]
    y = moving_average(y, 4)
    plt.plot(x, y, label='evaluation2')
    
    evaluation = np.load('log/lander/results_eval3.npy')
    x = np.arange(evaluation.shape[0])*25
    y = evaluation[:,0]
    y = moving_average(y, 4)
    plt.plot(x, y, label='evaluation3')
    """
